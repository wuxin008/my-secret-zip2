...
layout(std430, binding = 1) readonly buffer TMBuffer {
    mat4 TMs[];
};
...
void main() {
    ...
    position = TMs[(renderUbo.t) % 480] * position;
    gl_Position = renderUbo.proj * renderUbo.view * renderUbo.model * vec4(particle_norm(position.xyz, 1), 1.0);
